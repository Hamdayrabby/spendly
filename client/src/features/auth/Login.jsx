...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setServerError(err.response?.data?.error || "Failed to log in");
    }
  };

  const handleForgotPassword = (e) => {
    e.preventDefault();
    setServerError("Password reset is not configured. Please contact the administrator.");
  };

  return (
    <div className="min-h-screen w-full flex bg-background text-foreground overflow-hidden">
      
      {/* Left Side - The "Gravity" Visualization */}
      <div className="hidden lg:flex w-1/2 relative flex-col justify-center items-center overflow-hidden border-r border-border/50 bg-black/20">
        
        {/* Orbital Rings Background */}
        <div className="absolute inset-0 flex items-center justify-center opacity-20 pointer-events-none">
          <div className="absolute w-[600px] h-[600px] rounded-full border border-primary/40 animate-spin-slow" />
          <div className="absolute w-[800px] h-[800px] rounded-full border border-primary/20 animate-spin-slow-reverse" style={{ animationDuration: '70s' }} />
          <div className="absolute w-[1000px] h-[1000px] rounded-full border border-primary/10 animate-spin-slow" style={{ animationDuration: '100s' }} />
        </div>
        ...
        <div className="relative z-10 max-w-md px-12 text-center">
          ...
          <p className="text-muted-foreground text-lg font-light leading-relaxed">
            Spendly uses behavioral analytics and smart insights to help you understand where your money goes.
          </p>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={handleForgotPassword} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                    Forgot password?
                  </button>
                </div>
